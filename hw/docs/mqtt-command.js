...
export function createRobotControl({
  ...
  return {
    ...
    /**
     * 문 탐색 스캔. 오른쪽으로 step_deg 씩 steps 번 돌아 출발 방향에 서고, 그다음 직진.
     * hold_after_capture=1 이면 촬영마다 서서 scanContinue(rotation_deg) 를 기다린다
     * (진행 보고 scan_hold / scan_release). hold_timeout_s 는 3~60, 기본 20.
     */
    scan: ({ steps = 8, step_deg = 45, forward_m = 1.0, vx,
             hold_after_capture, hold_timeout_s } = {}) => {
      const params = { steps, step_deg, forward_m }
      if (vx != null) params.vx = vx
      if (hold_after_capture) params.hold_after_capture = 1
      if (hold_timeout_s != null) params.hold_timeout_s = hold_timeout_s
      const holdS = hold_after_capture
        ? steps * Math.min(60, Math.max(3, hold_timeout_s ?? 20)) : 0
      // 노드의 시한 산정과 같은 식으로 잡는다. 짧으면 정상 임무를 실패로 만든다.
      // 대기가 켜지면 촬영마다 hold_timeout_s(+사진 알림 대기)만큼 늘어난다.
      return run("scan_mission", params,
                 ((steps + 1) * 19 + forward_m / (vx || 0.15) * 4 + 19 + holdS
                  + (hold_after_capture ? steps * 9 : 0)) * 1000)
    },

    /**
     * 촬영 뒤 대기를 푼다 — "이 각도의 결과를 화면에 다 띄웠다". 로봇을 움직이지 않는다.
     * scan_hold 를 받은 판에서만 보낸다. 지난 각도·판 없음은 거부
     * (CommandRejected: stale_rotation / no_scan_in_progress).
     */
    scanContinue: (rotation_deg) => run("scan_continue", { rotation_deg }, 10000),
